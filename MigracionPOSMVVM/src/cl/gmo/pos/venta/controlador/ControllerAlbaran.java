...
public class ControllerAlbaran implements Serializable{
	...
	public void pagarAlbaran(){	
		...
		//String val_letras = "/^[A-Z a-z��. ]{3,50}$/";	
		/*
		if(!Pattern.matches(val_letras, devolucionForm.getDireccion())) {
			Messagebox.show("Debe Ingresar una direcci�n de cliente v�lida.");
			return;
		}
		
		if(!Pattern.matches(val_letras, devolucionForm.getNdireccion_cli())) {
			Messagebox.show("Debes ingresar el N� de Direcci�n del cliente.");
			return;
		}
	...
	private void cobrar_albaran(){
		...
		if(tipo_albaran.equals("DIRECTA")){
			...
			if(motivo.equals("0")) {
				Messagebox.show("Debe seleccionar un motivo de devoluci�n");
				return;
			}
			...
			if(tieneArmCrisContacto.equals("true")){
				...
				if(isController.equals("true") ){
					...
					Messagebox.show("Los productos son �pticos, solo un controller puede hacer la devoluci�n");
					return;
				}
			}else{
				//showPopWin("<%=request.getContextPath()%>/SeleccionPago.do?method=cargaFormularioCobroAlbaran&fecha="+fecha+"", 710, 285, vuelve_Pago_albaran_devolucion, false);
	...
	public void cerrar_ventanas(@BindingParam("win")  Window win) {
		...
		Messagebox.show("Se perderan todos los datos, Esta seguro de cancelar la venta?","Notificaci�n",
				Messagebox.YES|
				Messagebox.NO,
    ...
    public void creaPagoExitosoDevolucion(@BindingParam("seleccionPago")SeleccionPagoForm seleccionPago) {	
		...
		if(tmp[0].equals("0") || tmp[2].equals("true")){
			...
			Messagebox.show("Generando Nota de Cr�dito, espere un momento por favor....");
			
			/*Messagebox.show("Albaran", "Generando Nota de Cr�dito, espere un momento por favor...",
	...
	public void buscarAlbaranesDevo(){
		...
		if(cdg.equals("")){
			Messagebox.show("Debe ingresar un c�digo de albaran v�lido");
			respuesta = true;
		}else{
